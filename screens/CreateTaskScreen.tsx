...
const CreateTaskScreen: React.FC<CreateTaskScreenProps> = ({navigation, route}) => {
  ...
  const updateTimePeriod = (period: 'AM' | 'PM') => {
    const current = dueTime || new Date();
    ...
    <SafeAreaView style={styles.safe}>
            ...
            <View style={styles.pillRow}>
              {PRIORITIES.map((p) => (
                ...
                >
                  ...
                  <Text style={[styles.pillText, priority === p.key && {color: '#39391b'}]}>
                    {p.label}
                  </Text>
                </TouchableOpacity>
              ))}
                ...
                {allTags.map((t, idx) => (
                  ...
                  >
                    <MaterialCommunityIcons
                      name={(t.icon as any) || 'tag-outline'}
                      size={20}
                      color={tag === t.label ? '#52333c' : '#666643'}
                    />
                    <Text style={[styles.tagText, tag === t.label && styles.tagTextActive]}>
                      {t.label}
                    </Text>
                  </TouchableOpacity>
                ))}
      ...
      {Platform.OS === 'ios' && showDatePicker && (
        <Modal transparent animationType="fade" visible onRequestClose={closeDatePicker}>
          <Pressable style={styles.nativePickerOverlay} onPress={closeDatePicker}>
            <Pressable style={styles.nativePickerCard} onPress={() => {}}>
              <Text style={styles.nativePickerTitle}>Pick a date</Text>
              <DateTimePicker
                value={dueDate || new Date()}
                mode="date"
                display="spinner"
              ...
              <View style={styles.nativePickerButtons}>
                <TouchableOpacity style={[styles.nativePickerDone, styles.nativePickerCancel]} onPress={closeDatePicker}>
                  <Text style={styles.nativePickerDoneText}>Cancel</Text>
                </TouchableOpacity>
                <TouchableOpacity style={styles.nativePickerDone} onPress={closeDatePicker}>
                  <Text style={styles.nativePickerDoneText}>OK</Text>
                </TouchableOpacity>
              </View>
            </Pressable>
          </Pressable>
      ...
      {Platform.OS === 'ios' && showTimePicker && (
        <View style={styles.inlineTimePickerWrap}>
          <View style={styles.inlineTimePickerCard}>
            <View style={styles.inlineTimeHeader}>
              <Text style={styles.nativePickerTitle}>Pick a time</Text>
              <TouchableOpacity onPress={closeTimePicker} style={styles.inlineCloseBtn}>
                <MaterialCommunityIcons name="close" size={18} color="#7e5b64" />
              </TouchableOpacity>
            </View>
            <View style={styles.amPmRow}>
              <TouchableOpacity
                style={[styles.amPmChip, (dueTime || new Date()).getHours() < 12 && styles.amPmChipActive]}
                onPress={() => updateTimePeriod('AM')}
              >
                <Text style={[styles.amPmText, (dueTime || new Date()).getHours() < 12 && styles.amPmTextActive]}>AM</Text>
              </TouchableOpacity>
              <TouchableOpacity
                style={[styles.amPmChip, (dueTime || new Date()).getHours() >= 12 && styles.amPmChipActive]}
                onPress={() => updateTimePeriod('PM')}
              >
                <Text style={[styles.amPmText, (dueTime || new Date()).getHours() >= 12 && styles.amPmTextActive]}>PM</Text>
              </TouchableOpacity>
            </View>
            <DateTimePicker
              value={dueTime || new Date()}
            ...
            <View style={styles.inlineTimeActions}>
              <TouchableOpacity
                style={[styles.nativePickerDone, styles.nativePickerCancel]}
                onPress={closeTimePicker}
              >
                <Text style={styles.nativePickerDoneText}>Cancel</Text>
              </TouchableOpacity>
              <TouchableOpacity style={styles.nativePickerDone} onPress={closeTimePicker}>
                <Text style={styles.nativePickerDoneText}>OK</Text>
              </TouchableOpacity>
            </View>
          </View>
